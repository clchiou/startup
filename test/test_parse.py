...
class TestParseArgs(unittest.TestCase):
    ...
    def test_parse_args_many(self):
        ...
        arg_read_var = boot._parse_args(func, variables)
    ...
    def test_read_all_values(self):
        func = Mock({'x': ['var']})
        var = Variable()
        arg_read_var = boot._parse_args(func, {'var': var})
        self.assertEqual(1, len(arg_read_var))
    ...
    def test_wrong_anno(self):
        func = Mock({'x': ['x', 'y']})
        self.assertRaises(
            BootException, boot._parse_args, func, {})
...
class TestParseRet(unittest.TestCase):
    ...
    def test_parse_ret_0(self):
        func = Mock({})
        self.assertEqual(None, boot._parse_ret(func, {}))
    ...
    def test_parse_ret_1(self):
        var = Variable()
        func = Mock({'return': 'x'})
        self.assertEqual(var, boot._parse_ret(func, {'x': var}))
    ...
    def test_parse_ret_many(self):
        ...
        self.assertEqual(
            (var_x, var_x, var_y),
            boot._parse_ret(func, {'x': var_x, 'y': var_y}))

    def test_wrong_anno(self):
        func = Mock({'return': 1})
        self.assertRaises(
            BootException, boot._parse_ret, func, {})
